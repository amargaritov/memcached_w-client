...
#./install_location/bin/memcached -u -t $THREADS -m $(( $MEMORY * 1100 )) --disable-evictions --memory-limit=20480 --extended hashpower=20 --extended no_lru_crawler --extended no_lru_maintainer > $CUR_DIR/server.log 2>&1 & pid=$!
./install_location/bin/memcached -u -t $THREADS -m $(( $MEMORY * 1100 )) > $CUR_DIR/server.log 2>&1 & pid=$!
